...
set_tab_titlte_pos();setTimeout(function()
...
{min_width+=$(this).outerWidth();});tab_wrap.css('min-width',min_width);},set_tab_titlte_pos=function()
...
tabs.on('click',change_tab);tab_nav.on('click',switch_to_next_prev);win.on('debouncedresize',set_tab_titlte_pos);set_min_width();container.find('.av-active-tab-title').trigger('click');content_wrap.avia_swipe_trigger({prev:'.av_prev_tab_section',next:'.av_next_tab_section'});});};(function($)
